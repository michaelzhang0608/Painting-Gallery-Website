...
?>
  ...
  <ul class="login">
    ...
    foreach ($messages as $message) {
      echo "<li class=\"feedback\" style=\"color: white;position: absolute;
      top: 12px;
      right: 330px;\"><strong>" . htmlspecialchars($message) . "</strong></li>\n";
    } ?>
  </ul>
  ...
  <form class="login" action="<?php echo htmlspecialchars($action) ?>" method="post" novalidate>
    ...
    <input id="username" name="login_username" class="username" type="text" value="<?php echo htmlspecialchars($sticky_login_username); ?>" required />
    <label for="password" class="password_label">Password</label>
    <input id="password" name="login_password" class="password" type="password" required>
    <input name="login" class="login_submit" type="submit" value="Sign In">
  </form>
<?php
}
...
function create_account($db, $name, $username, $password, $password_confirmation)
{
  ...
  if (empty($username)) {
    ...
    $records = exec_sql_query(
      $db,
      "SELECT username FROM users WHERE (uname = :username);",
      array(
        ':username' => $username
      )
    )->fetchAll();
  ...
  if ($account_valid) {
    $result = exec_sql_query(
      $db,
      "INSERT INTO users (name, uname, pword) VALUES (:name, :username, :password);",
      array(
        ':name' => $name,
        ':username' => $username,
